...
import { Activity, Copy, LogOut, Menu, Network, Plus, Search, Send, Shield, Sparkles, Wifi, X } from 'lucide-react'
...
function App() {
  ...
  async function loadHistory() {
    setView('history'); setHistoryLoading(true); setError('')
    try {
      const r = await fetch(`${API}/history`, { headers: { Authorization: `Bearer ${token}` } })
      const data = await r.json()
      if (!r.ok) throw Error(data.detail || 'Unable to load history')
  ...
  async function diagnose() {
    ...
    const display = q
    setInput(''); setLoading(true); setError(''); setView('diagnostics')
  ...
  function newChat() { setMessages([]); setError(''); setView('diagnostics') }
  ...
  return <div className="app">
    <aside className={`sidebar ${drawer ? 'open' : ''}`}>
      ...
      <div className="side-section"><small>WORKSPACE</small><button className={view === 'diagnostics' ? 'side-active' : ''} onClick={() => setView('diagnostics')}><Sparkles size={16}/> AI Diagnostics</button><button className={view === 'history' ? 'side-active' : ''} onClick={loadHistory}><Search size={16}/> History</button><button><Shield size={16}/> Security</button></div>
      <div className="side-section"><small>MODE</small>{modes.map(x => <button key={x} className={mode === x ? 'mode-active' : ''} onClick={() => { setMode(x); setView('diagnostics') }}><Activity size={15}/>{x}</button>)}</div>
    ...
    <main className="main">
      <header><button className="icon-btn" onClick={() => setDrawer(true)}><Menu/></button><div className="header-title"><b>Network Debugger AI</b><span>{view === 'history' ? 'History' : mode}</span></div>{view === 'diagnostics' && <button className="share" onClick={share}><Copy size={15}/> Share</button>}</header>
      {view === 'history' ? <HistoryView history={history} loading={historyLoading} onRefresh={loadHistory}/> : <>
...
function HistoryView({ history, loading, onRefresh }) {
  return <section className="history-view"><div className="history-head"><div><h1>Investigation History</h1><p>Your previous network investigations.</p></div><button className="share" onClick={onRefresh}>Refresh</button></div>{loading ? <div className="history-empty">Loading history…</div> : history.length === 0 ? <div className="history-empty"><Search size={28}/><h3>No investigations yet</h3><p>Your completed diagnoses will appear here.</p></div> : <div className="history-list">{history.map(item => <article className="history-card" key={item.id}><div className="history-card-head"><b>{item.question?.replace(/^Mode:.*?User request:\s*/s, '') || 'Investigation'}</b><small>{item.created_at ? new Date(item.created_at).toLocaleString() : ''}</small></div><pre>{item.answer}</pre></article>)}</div>}</section>
}
